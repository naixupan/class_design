...
#读取数据集中的数据
train_data = pd.read_csv('data_1/train_public.csv')
test_public = pd.read_csv('data_1/test_public.csv')
train_inte = pd.read_csv('data_1/train_internet.csv')
...
def feature_Kmeans(data, label):
    mms = MinMaxScaler()
    feats = [f for f in data.columns if f not in ['loan_id', 'user_id', 'is_Default']]
    data = data[feats]
...
def fiterDataModel(data_, test_, y_, folds_):
    ...
    test_['is_Default'] = sub_preds

    return test_[['loan_id', 'is_Default']]
...
y = train_data['is_Default']
folds = KFold(n_splits=5, shuffle=True, random_state=546789)
IntePre = fiterDataModel(train_data, train_inteSame, y, folds)
IntePre['is_Def'] = train_inte['is_Default']
print(roc_auc_score(IntePre['is_Def'], IntePre.is_Default))
## 选择阈值0.05，从internet表中提取预测小于该概率的样本，并对不同来源的样本赋予来源值
InteId = IntePre.loc[IntePre.is_Default < 0.5, 'loan_id'].tolist()
train_inteSame['is_Default'] = train_inte['is_Default']
...
test_preds.rename({'loan_id': 'id'}, axis=1)[['id', 'is_Default']].to_csv('课设20204352218.csv', index=None)
